...
class _FlooringState extends State<Flooring> {
  ...
  /// **Fetch Flooring Image URLs from Firebase Storage**
  Future<void> fetchFlooringImages() async {
    try {
      final storageRef = FirebaseStorage.instance.ref("flooring");
      final ListResult result = await storageRef.listAll();

      if (result.items.isEmpty) {
        debugPrint("No images found in Flooring folder.");
        return;
      }
  ...
  /// **Save Selected Flooring Image URL to Firestore**
  Future<void> _saveSelectedFlooring() async {
    ...
    if (selectedIndex == null || imageUrls.isEmpty) {
      debugPrint("No flooring option selected.");
      return;
    }
    ...
    try {
      ...
      if (projectSnapshot.docs.isNotEmpty) {
        ...
        await userDoc.collection('projects').doc(projectId).update({
          'selectedFlooring': selectedImageUrl, // Save selected image URL
          'updatedAt': FieldValue.serverTimestamp(),
        });

        debugPrint("Flooring selection saved: $selectedImageUrl");
      } else {
        debugPrint("No projects found for user.");
      }
    } catch (e) {
      debugPrint("Error saving flooring selection: $e");
    }
  }
